...
export async function POST(req) {
  try {
    ...
    const { personAIncome, personBIncome, cohabitationDate, separationDate, children, taxYear, label, personAName, personBName } = body

    if (!personAIncome || !personBIncome || !cohabitationDate || !separationDate) {
      return NextResponse.json({ error: 'All fields required' }, { status: 400 })
    }
    if (!children?.length) {
      return NextResponse.json({ error: 'At least one child required' }, { status: 400 })
    }
    ...
    const inputs = {
      personAIncome: +personAIncome, personBIncome: +personBIncome,
      cohabitationDate, separationDate, children,
      taxYear: taxYear || 2023, personAName, personBName,
    }
    const results = calculateWithChildSupport(inputs)
    ...
    const saved = await pool.query(
      ...
      [user.id, 'with_child', label || null, JSON.stringify(inputs), JSON.stringify(results)]
    )
