    ...
    if (isSubscription) {
      ...
      try {
        ...
        if (fullSession.line_items?.data?.[0]?.price?.product) {
          ...
          if (productMetadata.agreement_type) {
            agreementType = productMetadata.agreement_type;
          }
        }
        ...
        if (customerError || !customerData) {
          console.error('Error fetching user from customer:', customerError);
          return;
        }
        ...
        console.info(`Successfully processed one-time payment and granted ${agreementType} access for session: ${checkout_session_id}`);
      } catch (error) {
