...
export const COMMUNICATION_TEMPLATES = {
  app_weekly: {
    label: 'Scheduled written updates and weekly check-ins',
    variables: ['urgentResponseTime'],
    template: ({ urgentResponseTime, party1, party2 }) =>
      `${party1} and ${party2} shall communicate about the children through written updates and weekly check-ins regarding the children's well-being, schooling, and activities. Urgent matters shall be addressed within ${urgentResponseTime}.`,
  },
  email_biweekly: {
  ...
  combined: {
    label: 'Combined — messages for logistics, email for formal matters',
    variables: ['emailAddress', 'urgentResponseTime'],
    template: ({ emailAddress, urgentResponseTime, party1, party2 }) =>
      `${party1} and ${party2} shall use written messages for day-to-day logistics regarding the children, and email at ${emailAddress} for formal matters requiring documentation. Urgent matters shall be addressed within ${urgentResponseTime}.`,
  },
}
...
export const SPECIAL_CLAUSE_TEMPLATES = {
  right_first_refusal: {
    ...
    icon: '👶',
    description: 'If one parent needs childcare for more than a set number of hours during their parenting time, they must offer the other parent the chance to care for the children first before hiring a sitter.',
    variables: ['hours'],
    defaults: { hours: 4 },
    template: ({ hours }) =>
  ...
  relocation: {
    ...
    icon: '📍',
    description: 'Limits how far either parent can move with the children without giving the other parent advance written notice and consent (or a court order).',
  ...
  travel: {
    ...
    icon: '✈️',
    description: 'Requires the written consent of the other parent before either parent can travel internationally with the children, and a commitment to sign Travel Consent Letters.',
    variables: [],
    template: () =>
  ...
  new_partners: {
    ...
    icon: '💞',
    description: 'Sets a minimum length of relationship before either parent can introduce a new romantic partner to the children.',
    variables: ['months'],
    defaults: { months: 6 },
    template: ({ months }) =>
  ...
  social_media: {
    label: 'Social Media',
    icon: '📱',
    description: 'Stops either parent from posting photos or identifying information about the children on public social media without the other parent\'s written consent.',
    variables: [],
    template: () =>
  ...
  other: {
    ...
    icon: '✏️',
    description: 'Write your own clause in plain language. Use this for anything specific to your family that isn\'t covered by the other options.',
