...
const BASE_CSS = `
  * { box-sizing: border-box; }
  ...
  /* Body text */
  /* Slightly looser line-height + small padding so the rasterizer's
     page-slice gutter has room to land on a clean inter-line gap
     instead of mid-text. */
  p { margin: 6pt 0; font-size: 10.5pt; line-height: 1.55; text-align: justify; padding-bottom: 1pt; }
  p.clause { margin: 6pt 0; padding-left: 24pt; text-indent: -24pt; padding-bottom: 2pt; }
  li, .indent, .indent-2 { line-height: 1.55; padding-bottom: 1pt; }
  p.clause .num {
  ...
  /* Schedule break — each schedule starts on a fresh page in PDF.
     A small top buffer guarantees the schedule title never sits flush
     against the canvas slice point. */
  .schedule {
  ...
  /* Keep card-like blocks together — small enough to fit on one page,
     visually broken if split. Paragraphs and list items are deliberately
     ALLOWED to split across pages — combined with the tight line-height
     and generous top/bottom page margins, splits land in inter-line
     whitespace rather than mid-glyph, and we avoid the "huge blank gap
     at top of page" cascade that happens when too many adjacent blocks
     are marked unsplittable. */
  .calc-box,
  .eq-card,
  .warning-callout,
  .declaration,
  ...
  .eq-card .title {
    ...
    letter-spacing: 0.04em;
    margin-bottom: 16pt;
    /* Force pure white + a subtle drop-shadow so the title reads
       cleanly over the gradient when html2canvas rasterizes it.
       Without these, the heading appeared semi-transparent in the
       exported PDF. */
    color: #ffffff !important;
    text-shadow: 0 1pt 2pt rgba(0, 0, 0, 0.22);
...
function renderParenting(bundle, secNum = 3) {
  ...
  if (sched?.transportation_template) {
    ...
    if (sched.pickup_dropoff_location) {
      body += clause(`${n}.6.2`, `The default location for exchanges shall be ${sched.pickup_dropoff_location}, unless the parties agree otherwise in writing.`)
    }
  }
...
function renderSignatureBlock(bundle, secNum = 8) {
  ...
  const a = bundle.agreement
  const { party1Full, party2Full } = getNamesForBundle(bundle)
  const signingCity = a.signing_city && String(a.signing_city).trim()
    ? escapeHtml(String(a.signing_city).trim())
  ...
  return `
    ...
    ${clause(`${n}.2`, `SIGNED at ${signingCity}, Ontario.`)}
    ${clause(`${n}.3`, `SIGNED, SEALED AND DELIVERED in the presence of:`)}
...
export function renderScheduleB_Parenting(bundle) {
  ...
  // Transition Days detail table — pickup/dropoff times and locations
  const dayLabels = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat']
  const transitions = sched.regular_schedule_variables?.transitions || {}
  const transitionRows = []
  for (let w = 0; w < 4; w++) {
    for (let d = 0; d < 7; d++) {
      if (weeks[w][d] !== 'transition') continue
      const detail = transitions[`${w}-${d}`] || {}
      const hasAny = detail.pickup_time || detail.pickup_location || detail.dropoff_time || detail.dropoff_location
      if (!hasAny) continue
      transitionRows.push(`<tr>
        <td>Week ${w + 1}</td>
        <td>${dayLabels[d]}</td>
        <td>${detail.pickup_time || '—'}</td>
        <td>${detail.pickup_location || '—'}</td>
        <td>${detail.dropoff_time || '—'}</td>
  ...
  if (transitionRows.length > 0) {
    transitionTable = `
      <h4 style="margin: 18pt 0 6pt 0; font-size: 11pt;">Transition Day Details</h4>
      <table class="schedb-transitions" style="width:100%; border-collapse: collapse; font-size: 9.5pt;">
        <thead>
          <tr style="background: #f4f4f4;">
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Week</th>
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Day</th>
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Pickup Time</th>
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Pickup Location</th>
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Dropoff Time</th>
            <th style="border:1px solid #ccc; padding:6pt; text-align:left;">Dropoff Location</th>
          </tr>
        </thead>
        <tbody>
          ${transitionRows.map((r) => r.replace(/<td>/g, '<td style="border:1px solid #ccc; padding:6pt;">')).join('')}
        </tbody>
      </table>`
  }

  // Default pickup/dropoff location (set on the schedule card)
  let defaultLocationLine = ''
  if (sched.pickup_dropoff_location) {
    defaultLocationLine = `<p style="margin-top: 14pt; font-size: 10pt;"><strong>Default exchange location:</strong> ${sched.pickup_dropoff_location}</p>`
  }
