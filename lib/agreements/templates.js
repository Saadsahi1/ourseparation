...
function renderChildSupport(bundle) {
  ...
  if (sc.child_support_payor === 'none') {
    body += clause('4.1.1', `The parties acknowledge their respective incomes and parenting arrangements and agree that no monthly child support is payable at this time. Either party may seek child support at any time in accordance with the Federal Child Support Guidelines, O. Reg. 391/97.`)
  } else if (amount > 0 && payor && recipient) {
    body += clause('4.1.1', `${payor} shall pay ${recipient} child support in the amount of <strong>${fmtCAD(amount)} per month</strong>, payable on the first day of each month, commencing on the first day of the month following the execution of this Agreement, for the benefit of ${childNames || 'the children'}.`)
    body += clause('4.1.2', `Payment shall be by direct deposit or electronic transfer to ${recipient}'s designated account, or by such other means as the parties may agree in writing.`)

    // 4.1.3 — Detailed calculation basis using actual incomes
    if (arrangement === 'section9') {
      body += clause('4.1.3', `This amount is calculated under Section 9 of the Federal Child Support Guidelines (shared parenting set-off) based on:`)
      body += '<ul class="legal">'
      ...
      if (sc.section9_adjustment_notes) {
        body += clause('4.1.4', `Adjustments under Section 9(c) (increased costs, transportation, conditions/means/needs): ${sc.section9_adjustment_notes}`)
      }
    } else {
      const payorIncome = sc.child_support_payor === 'party1' ? p1Inc : p2Inc
      body += clause('4.1.3', `This amount is the Table amount under Section 3 of the Federal Child Support Guidelines, based on ${payor}'s annual income of <strong>${fmtCAD(payorIncome)}</strong> and ${children.length} child${children.length === 1 ? '' : 'ren'} in Ontario.`)
    }
    ...
    if (sc.arrears_amount && sc.arrears_owed_to) {
      ...
      body += clause('4.1.5', `${arrearsPayor} shall pay ${arrearsRecipient} child support arrears in the amount of <strong>${fmtCAD(sc.arrears_amount)}</strong> within ${days} days of the execution of this Agreement.`)
    }
  }
...
function renderSpousalSupport(bundle) {
  ...
  if (sc.spousal_support_payor === 'none' || tplKey === 'complete_release') {
    ...
    body += clause('5.1.1', `${payor} shall pay ${recipient} spousal support of <strong>${fmtCAD(amountMo)} per month</strong>, payable on the first day of each month, commencing on the first day of the month following the execution of this Agreement.`)

    // 5.1.2 — Basis for the amount
    body += clause('5.1.2', `The parties acknowledge they have considered the Spousal Support Advisory Guidelines (SSAG) based on:`)
    body += '<ul class="legal">'
    ...
    body += clause('5.1.3', tpl.template({
    ...
    const taxSec = triggers.length > 0 ? '5.3' : '5.2'
...
function renderProperty(bundle) {
  ...
  body += clause('6.1.2', `Based on those statements:`)
  body += '<ul class="legal">'
  ...
  if (effectiveAmount > 0 && payorName) {
    ...
    if (hasOverride) {
      body += clause('6.2.1', `The calculated equalization payment pursuant to section 5 of the Family Law Act is <strong>${fmtCAD(calcEq)}</strong>. The parties have agreed, however, that <strong>${payorName}</strong> shall pay <strong>${recipientName}</strong> an equalization payment of <strong>${fmtCAD(effectiveAmount)}</strong>.${division.custom_equalization_notes ? ` ${division.custom_equalization_notes}` : ''}`)
    } else {
      body += clause('6.2.1', `Pursuant to section 5 of the Family Law Act, <strong>${payorName}</strong> shall pay <strong>${recipientName}</strong> an equalization payment of <strong>${fmtCAD(effectiveAmount)}</strong>.`)
    }
    ...
    body += clause('6.2.1', `The parties' Net Family Property values are equal, and no equalization payment is owing under section 5 of the Family Law Act.`)
  }
...
function renderGeneralProvisions(bundle) {
  ...
  body += clause(`7.${n}.1`, `Each party acknowledges that:`)
  body += '<ul class="legal">'
  ...
  body += subsection(`7.${n}`, 'Full Financial Disclosure')
  body += clause(`7.${n}.1`, `Each party warrants that they have provided to the other a full, complete, and accurate disclosure of all significant assets, debts, and other financial obligations as of the date of separation, as set out in Schedule A and Schedule C. The parties acknowledge that nondisclosure or misrepresentation of significant financial information may render this Agreement voidable under section 56(4)(a) of the Family Law Act.`)
  n++

  body += subsection(`7.${n}`, 'Variation on Material Change')
  body += clause(`7.${n}.1`, `Provisions of this Agreement relating to child support and parenting may be varied where there has been a material change in circumstances, in accordance with the Federal Child Support Guidelines, the Divorce Act, and the Children's Law Reform Act. Spousal support may likewise be varied upon a material change in circumstances unless this Agreement provides otherwise.`)
  n++

  body += subsection(`7.${n}`, 'Entire Agreement')
  body += clause(`7.${n}.1`, `This Agreement, including all schedules attached, constitutes the entire agreement between the parties with respect to its subject matter and supersedes all prior negotiations, representations, and agreements, whether written or oral. There are no representations, warranties, or conditions affecting this Agreement other than those expressed herein.`)
  n++

  body += subsection(`7.${n}`, 'Governing Law and Jurisdiction')
  body += clause(`7.${n}.1`, `This Agreement shall be governed by and construed in accordance with the laws of the Province of Ontario and the laws of Canada applicable therein. The parties attorn to the exclusive jurisdiction of the courts of Ontario for the determination of any matter arising under this Agreement.`)
  n++

  body += subsection(`7.${n}`, 'Severability')
  body += clause(`7.${n}.1`, `If any provision of this Agreement is held by a court of competent jurisdiction to be invalid, illegal, or unenforceable, that provision shall be deemed severed from this Agreement, and the remaining provisions shall continue in full force and effect.`)
  n++

  body += subsection(`7.${n}`, 'Amendments')
  body += clause(`7.${n}.1`, `No amendment, modification, waiver, or release of this Agreement or any of its provisions shall be binding on either party unless made in writing, signed by both parties, and witnessed in the manner required by section 55(1) of the Family Law Act.`)
  n++

  body += subsection(`7.${n}`, 'Counterparts and Electronic Execution')
  body += clause(`7.${n}.1`, `This Agreement may be executed in any number of counterparts, each of which when so executed shall be deemed to be an original and all of which taken together shall constitute one and the same instrument. Delivery of an executed counterpart by electronic transmission (including PDF) shall be as effective as delivery of a manually executed counterpart.`)
  n++

  body += subsection(`7.${n}`, 'Time of the Essence')
  body += clause(`7.${n}.1`, `Time shall be of the essence in this Agreement.`)
  n++

  body += subsection(`7.${n}`, 'Further Assurances')
  body += clause(`7.${n}.1`, `Each party shall execute and deliver all such further documents, instruments, and assurances, and shall do all such further acts and things, as may be reasonably required to give full effect to the terms and intent of this Agreement.`)
  n++

  body += subsection(`7.${n}`, 'Binding Effect')
  body += clause(`7.${n}.1`, `This Agreement shall enure to the benefit of and be binding upon the parties and their respective heirs, executors, administrators, legal representatives, successors, and permitted assigns.`)
  n++
...
function renderTableOfContents(bundle) {
  ...
  const a = bundle.agreement
  const items = bundle.propertyItems || []
  const tt = bundle.additionalTerms || {}

  const entries = [
    { num: '1', label: 'Background' },
    { num: '2', label: 'Definitions and Interpretation' },
  ]
  if (children.length > 0) entries.push({ num: '3', label: 'Parenting Arrangements' })
  if (children.length > 0 && sc.child_support_payor) entries.push({ num: '4', label: 'Child Support' })
  if (sc.spousal_support_payor) entries.push({ num: '5', label: 'Spousal Support' })
  if (a.agreement_type !== 'prenup' && a.agreement_type !== 'cohabitation') {
    entries.push({ num: '6', label: 'Property Division and Equalization' })
  }
  entries.push({ num: '7', label: 'General Provisions' })
  entries.push({ num: '8', label: 'Signatures' })
