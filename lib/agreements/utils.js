...
// agreement: the agreements row. user: the logged-in user (used as fallback
// for Party 1 when the agreement-level name fields aren't set yet).
// Preference order:
//   Party 1: agreement.party1_first_name + party1_last_name
//          → user.first_name + user.last_name
//          → user.email
//          → 'Party 1'
//   Party 2: agreement.party2_first_name + party2_last_name
//          → legacy agreement.party2_name
//          → 'Party 2'
...
export const getPartyDisplayName = (agreement, position, user) => {
  const join = (f, l) => [f, l].filter((s) => s && String(s).trim()).join(' ').trim()
  let raw
  if (position === 'party1') {
    raw = join(agreement?.party1_first_name, agreement?.party1_last_name)
    if (!raw && user && (user.first_name || user.last_name)) {
      raw = join(user.first_name, user.last_name)
    }
    if (!raw && user?.email) raw = user.email
    if (!raw) raw = 'Party 1'
  } else {
    raw = join(agreement?.party2_first_name, agreement?.party2_last_name)
    if (!raw) raw = agreement?.party2_name || ''
    if (!raw) raw = 'Party 2'
  }
