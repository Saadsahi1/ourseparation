// lib/db/schema.js
// Runs on every server start via instrumentation.js. Uses an advisory lock
// + a fast pre-check so concurrent Vercel cold starts don't fight each
// other over Postgres's system catalogs (which was causing 40P01 deadlocks
// when a real INSERT happened to overlap with two cold-starts running the
// schema migration at the same time).

import { Pool } from 'pg'

// Random but stable bigint used with pg_try_advisory_lock so only one
// process in the cluster runs the migration at a time.
const SCHEMA_LOCK_KEY = '8923471027634987234'
...
async function ensureSchema() {
  ...
  // Fast path: if the schema looks already-migrated, skip all DDL. This
  // is what runs 99% of the time and eliminates the deadlock window.
  try {
    const probe = await client.query(`
      SELECT
        EXISTS(SELECT 1 FROM information_schema.tables  WHERE table_name = 'agreements')          AS has_agreements,
        EXISTS(SELECT 1 FROM information_schema.tables  WHERE table_name = 'children')            AS has_children,
        EXISTS(SELECT 1 FROM information_schema.tables  WHERE table_name = 'parenting_terms')     AS has_parenting,
        EXISTS(SELECT 1 FROM information_schema.tables  WHERE table_name = 'support_calculations') AS has_support,
        EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'agreements'
                AND column_name = 'interview_data')                                                AS has_old_schema,
        EXISTS(SELECT 1 FROM information_schema.columns WHERE table_name = 'agreements'
                AND column_name = 'party2_first_name')                                             AS has_party_split
    `)
    const r = probe.rows[0] || {}
    if (r.has_agreements && r.has_children && r.has_parenting && r.has_support && !r.has_old_schema && r.has_party_split) {
      console.log('✅ Schema already migrated — skipping DDL')
      client.release()
      await pool.end()
      return
    }
    // Fast incremental path: tables exist and shape is current, but a newly
    // added column (party_split set) is missing. Run only the additive
    // column migrations rather than the full DDL pass, so we never get
    // stuck on a "missing column" error in prod between deploys.
    if (r.has_agreements && r.has_children && r.has_parenting && r.has_support && !r.has_old_schema && !r.has_party_split) {
      console.log('⚙ Adding missing party-split columns (incremental migration)')
      await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party1_first_name VARCHAR(120)`).catch((e) => console.warn('party1_first_name:', e.message))
      await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party1_last_name  VARCHAR(120)`).catch((e) => console.warn('party1_last_name:', e.message))
      await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party2_first_name VARCHAR(120)`).catch((e) => console.warn('party2_first_name:', e.message))
      await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party2_last_name  VARCHAR(120)`).catch((e) => console.warn('party2_last_name:', e.message))
      await client.query(`
        UPDATE agreements
        SET party2_first_name = split_part(party2_name, ' ', 1),
            party2_last_name  = NULLIF(
              CASE WHEN position(' ' IN party2_name) > 0
                   THEN substring(party2_name FROM position(' ' IN party2_name) + 1)
                   ELSE ''
              END, '')
        WHERE party2_first_name IS NULL
          AND party2_name IS NOT NULL
          AND length(trim(party2_name)) > 0
      `).catch((e) => console.warn('party2 backfill:', e.message))
      await client.query(`
        UPDATE agreements a
        SET party1_first_name = COALESCE(NULLIF(u.first_name, ''), NULL),
            party1_last_name  = COALESCE(NULLIF(u.last_name,  ''), NULL)
        FROM users u
        WHERE a.user_id = u.id
          AND a.party1_first_name IS NULL
      `).catch((e) => console.warn('party1 backfill:', e.message))
    ...
    console.warn('Schema probe failed, will attempt migration:', e.message)
  }

  // Slow path: acquire an advisory lock so only one process runs the
  // migration at a time. If another process already holds the lock,
  // we skip — they'll finish in a moment.
  try {
    const lockRes = await client.query(`SELECT pg_try_advisory_lock($1::bigint) AS got`, [SCHEMA_LOCK_KEY])
    if (!lockRes.rows[0]?.got) {
      console.log('⏳ Another process is running the schema migration — skipping')
      client.release()
      await pool.end()
      return
    }
  } catch (e) {
    console.warn('Advisory lock failed:', e.message)
    client.release()
    await pool.end()
    return
  }
  ...
  try {
    ...
    // 2026-06: split party names into first / last while keeping legacy
    // party2_name as the computed display string for backwards-compat.
    await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party1_first_name VARCHAR(120)`).catch(() => {})
    await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party1_last_name  VARCHAR(120)`).catch(() => {})
    await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party2_first_name VARCHAR(120)`).catch(() => {})
    await client.query(`ALTER TABLE agreements ADD COLUMN IF NOT EXISTS party2_last_name  VARCHAR(120)`).catch(() => {})
    // One-time backfill: split any existing party2_name on the first space so
    // pre-existing agreements continue to display the same name in the editor.
    // Idempotent — only fires when the new columns are still NULL.
    await client.query(`
      UPDATE agreements
      SET party2_first_name = split_part(party2_name, ' ', 1),
          party2_last_name  = NULLIF(
      ...
      WHERE party2_first_name IS NULL
        ...
        AND length(trim(party2_name)) > 0
    `).catch(() => {})
    // Backfill Party 1 from the owning user's first/last name when not yet
    // set on the agreement.
    await client.query(`
      UPDATE agreements a
      SET party1_first_name = COALESCE(NULLIF(u.first_name, ''), NULL),
          party1_last_name  = COALESCE(NULLIF(u.last_name,  ''), NULL)
    ...
    `)
    // Drop NOT NULL constraints in case the older schema is already present
    await client.query(`ALTER TABLE children ALTER COLUMN full_name DROP NOT NULL`).catch(() => {})
    await client.query(`ALTER TABLE children ALTER COLUMN birth_date DROP NOT NULL`).catch(() => {})
    ...
    try { await client.query(`SELECT pg_advisory_unlock($1::bigint)`, [SCHEMA_LOCK_KEY]) } catch {}
    client.release()
    await pool.end()
  }
}
