...
function calcCCB(afni, children, parent, P) {
  ...
  for (const c of children) {
    ...
    const share = c.residesWith === 'shared' ? 0.5 : 1
    if (c.residesWith === parent || c.residesWith === 'shared') {
      if (age < 6) under6 += share
      else if (age < 18) age6to17 += share
    }
  }
...
export function calculateTax({
  ...
  const ontEligibleDependant = calcEligibleDependantCredit(children, parent, P) / P.fedRates[0] * P.ontRates[0] // Scale to Ontario rate
